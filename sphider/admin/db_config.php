<?php
		...
		if (isset($_GET['p'])) {
			$p = (substr(trim(($_GET['p'])),0,2));
		} else {
			$p = '99';		//	obligatory new login is required
		}
...
//echo "\r\n\r\n<br>Array:<br><pre>";print_r($data);echo "</pre>\r\n";
		...
		if (isset($db_count))
        $db_count = cleaninput(substr(trim($db_count),0,2));
        ...
        if (!isset($f) || $f == 98) {
            ...
            echo "<form name='dobackup' id='dbform1' method='get' action='admin.php'>
        ...
        ";
            ...
            if (isset($Submit)) {
                ...
                if (!is_writable("$settings_dir/database.php")) {
                    print "Database configuration file is not writeable, chmod 777 .../settings/database.php under *nix systems";
                } else {
                    //  Check, whether database configuration file is really writeable.
                    //  Store test string and try to read it afterwards.
                    //  Test string will be automatically deleted on next store of real database settings,
                    //  as final storage starts to save all at the beginning of config file.
                    $string = "<?php echo '<br />db config file check1 $_database1 database1 = '$_database1'<br />\r\n'; ?>";
                    $handle     = fopen("$settings_dir/database.php", "wb");
                    $written    = fwrite($handle, $string);
                    fclose($handle);

                    $handle = fopen("$settings_dir/database.php", "rb");
                    $cont 	= '';
                    $cont   = stream_get_contents($handle);
                    fclose($handle);

                    // if database.php write failure
                    if (!strstr($cont, "db config file check")) {
                        echo "        <br />
                            <p class='warnadmin cntr'><br />
                            Attention: Sphider-plus is unable to write into database configuration file
                            <br /><br />$settings_dir/database.php.<br />
                            <br />
                    ...
                    //  write new values into intermediate database config file
                    $fhandle=fopen("$settings_dir/db_int.php","wb");
                    fwrite($fhandle,"<?php \n");
                    ...
                    echo "<br />
                            <a class='bkbtn' href='db_config.php?p=98' title='Reload Settings'>Complete this process</a>
                            <p>\n\n</p>
                            </div></div></div>
                            </body>
                            </html>
            ...
            <br />
            <div id='settings'>
                <form class='txt' name='form1' method='get' action='db_config.php'>
                    ...
                    <fieldset><legend><a name=\"set_1\">[ Database 1 settings ]</a></legend><br />
                        ...
                        <input class='hide' type='hidden' name='db_count' id='db_count' value='$db_count'/>
                        <input class='hide' type='hidden' name='p' id='ip1' value='98'/>
            ...
            if (!$tables1) {
                echo "
            <form class='cntr' name='form1a' method='get' action='install_tables.php'>
                <fieldset>
                    <label for='submit1a' class='sml'>Install all tables for database 1:</label>
						<input class='hide' type='hidden' name='f' id='f' value='0'/>
						<input class='hide' type='hidden' name='db_num' id='db_num1' value='1'/>
        ...
        ";
            ...
            if ($tables1) {
                //      try to allocate db2
                echo "
                <form class='txt' name='form2' method='get' action='db_config.php'>
                    <input class='hide' type='hidden' name='Submit' value='1'/>
                    <br />
                    <fieldset><legend><a name=\"set_2\">[ Database 2 settings ]</a></legend><br />
                        ...
                        <input class='hide' type='hidden' name='p' id='ip2' value='98'/>
                ...
                if (!$tables2) {
                    echo "
                <form class='txt' name='form2a' method='get' action='install_tables.php'>
                    <fieldset>
                        <label for='submit2a' class='sml'>Install all tables for database 2:</label>
						<input class='hide' type='hidden' name='f' id='f' value='0'/>
						<input class='hide' type='hidden' name='db_num' id='db_num2' value='2'/>
            ...
            if ($tables2) {
                //      try to allocate db3
                echo "
                <form class='txt' name='form3' method='get' action='db_config.php'>
                    <input class='hide' type='hidden' name='Submit' value='1' />
                    <br />
                    <fieldset><legend><a name=\"set_3\">[ Database 3 settings ]</a></legend><br />
                        ...
                        <input class='hide' type='hidden' name='p' id='ip3' value='98'/>
                ...
                if (!$tables3) {
                    echo "
                <form class='cntr' name='form3a' method='get' action='install_tables.php'>
                    <fieldset>
                        <label for='submit' class='sml'>Install all tables for database 3:</label>
						<input class='hide' type='hidden' name='f' id='f' value='0'/>
						<input class='hide' type='hidden' name='db_num' id='db_num3' value='3'/>
            ...
            if ($tables3) {
                //      try to allocate db4
                echo "
                <form class='txt' name='form4' method='get' action='db_config.php'>
                    <input class='hide' type='hidden' name='Submit' value='1'/>
                    <br />
                    <fieldset><legend><a name=\"set_4\">[ Database 4 settings ]</a></legend><br />
                        ...
                        <input class='hide' type='hidden' name='_db_count' id='_db_count' value='$db_count' />
                        <input class='hide' type='hidden' name='p' id='ip4' value='98'/>
                ...
                if (!$tables4) {
                    echo "
                <form class='cntr' name='form4a' method='get' action='install_tables.php'>
                    <fieldset>
                        <label for='submit4a' class='sml'>Install all tables for database 4:</label>
						<input class='hide' type='hidden' name='f' id='f' value='0'/>
						<input class='hide' type='hidden' name='db_num' id='db_num4' value='4'/>
            ...
            if ($tables4) {
                ...
                <form class='txt' name='form5' method='post' action='db_config.php'>
                    ...
                    <fieldset><legend><a name=\"set_5\">[ Database 5 settings ]</a></legend><br />
                        ...
                        <input class='hide' type='hidden' name='_db_count' id='_db_count' value='$db_count'/>
                        <input class='hide' type='hidden' name='p' id='ip5' value='98'/>
                ...
                if (!$tables5) {
                    echo "
                <form class='cntr' name='form5a' method='get' action='install_tables.php'>
                    <fieldset>
                        <label for='submit4a' class='sml'>Install all tables for database 5:</label>
						<input class='hide' type='hidden' name='f' id='f' value='0'/>
						<input class='hide' type='hidden' name='db_num' id='db_num5' value='5'/>
        ...
        echo "
            ...
            <div class='clear'>
                <a class='navup' href='db_config.php?p=98' title='Jump to Page Top'>Top</a>
            </div>
            <br />
            <br />
        </body>
    ...
    function check_dbs($output, $mysql_table_prefix1, $mysql_table_prefix2, $mysql_table_prefix3, $mysql_table_prefix4, $mysql_table_prefix5) {
        ...
        require "$settings_dir/db_int.php";
        ...
        //      in order to get fresh status values, reset secundary db's
        $db1_set    = '1';
        $db2_set    = '0';
        $db3_set    = '0';
        $db4_set    = '0';
        ...
        if (!is_writable("$settings_dir/database.php")) {
            print "Intermediate database file is not writable, chmod 777 .../admin/db_config.php under *nix systems";
        } else {
            $fhandle=fopen("$settings_dir/database.php","wb");
            fwrite($fhandle,"<?php \n");
		...
		@unlink("$settings_dir/db_int.php");

<?php
    ...
    if ($link_id > '0') {     //      if valid link_id exists
        ...
        preg_match("@<head[^>]*>(.*?)<\/head\s*>@si",$raw_file, $regh);         //get head part of HTML only
        preg_match("@<body[^>]*>(.*?)<\/body\s*>@si",$raw_file, $regs);         //get body part only
        ...
        //  find externally hosted images for youtbe.com
        if (strpos($url, "youtube.com") && $ex_media == "1") {

            //  try to get thumbnail from any video on this URL
            preg_match("@<meta property=\"og\:image\" content=\"(.*?)\">@si", $regh[1], $regth);

            //  get the name of this image
            preg_match("@<meta property=\"og\:title\" content=\"(.*?)\">@si", $regh[1], $regti);

            $this_media[0]  = $regti[1];
            $this_media[1]  = "alt=\"".$title."\" src= \"".$regth[1]."\"";
            //  add this image to 'all media' array
            $all_media[]    = $this_media;

            //  now any other image on this Youtube URL
            preg_match_all("/data-thumb=\"(.*?).jpg\"/si", $raw_file, $regs, PREG_SET_ORDER);

            foreach ($regs as $val) {
                $this_media = array();

                //  build the title for this image (image ID)
                if (strpos($val[1], "default")) {
                    $title0 = substr($val[1], 0, strrpos($val[1], "/default"));
                    $title0 = substr($title0, strrpos($title0, "/")+1);

                    $rest0   = substr($raw_file, strpos($raw_file, "$title0"));
                    $rest1   = substr($rest0, strpos($rest0, "u0026id=".$title0.""), 800);

                    preg_match("@u0026title=(.*?)\\\u0026@si",$rest1, $regt);

                    //  second attempt to find any title
                    if (!$regt[1]) {
                        preg_match("@title=\"(.*?)\"@si",$rest1, $regt);
                    }
                    $title = str_replace("+", " ", $regt[1]);

                    //  no title found, we need to use the video ID as title, or we call it default
                    if (!$title) {
                        if (!$title0) {
                            $title = rand().".jpg";
                        } else {
                            $title = "Image ID: ".$title0;
                        }
                    }
                    $this_media[0]  = $title;
                } else {
                    $this_media[0]  = substr($val[0], strpos($val[0], "thumb=")+6);
                }

                //  create the linkk (src = ....) for this image
                if  (!strpos($val[1], "ttp")) {
                    $this_media[1] = "alt=\"".$title."\" src= \"https:".$val[1].".jpg\"";
                } else {
                    $this_media[1] = "alt=\"".$title."\" src= \"".$val[1]."\"";
                }
...
//  ********   find and index images     ********
        if ($index_image == '1') {
			...
			//	first process all images found in PDFs
			if (strpos ($clean_url, ".pdf")) {
				$width_p	= '160';		// 	width of PDF thumb image
				$height_p	= '100';		//	height of PDF thumb image

				$files	=scandir($tmp_dir);
//echo "\r\n\r\n<br>files Array:<br><pre>";print_r($files);echo "</pre>\r\n";
				foreach ($files as $image) {
					if (strpos($image, "pdf_jpg")) {
						//	get some basic info about this new image
						$image_path = $tmp_dir."/".$image;
						$suffix 	= trim(strtolower(substr($image_path, strrpos($image_path, ".")+1)));
						$new_md5    = md5_file($image_path);
						$linkparts  = parse_url($image);
						$my_name    = $image;	// as the images in PDFs are untitled
						$my_name	= str_replace("_jpg", "", $my_name);
						$link 		= $url;    //  as an individual image is not addressable in PDF documents

						//      try to find already indexed images with the same md5sum
						...
						if ($result->num_rows && $new_md5) {   //      enter only, if PHP function md5_file()  is allowed for remote files
							...
							if ($dup_media == '1') {    //  if we should index duplicate media on different pages
								...
								if ($result->num_rows) {
									...
									$sql_link       = $sql_url;	//	use the PDF URL to find the image
									$thumb          = $var[4];
							...
							//  for new image objects we need to process all details
							$size = @getimagesize ($image_path);  // get geometrical info of temporary image
							...
							if ($size && ($size[0] >= $min_image_x && $size[1] >= $min_image_y)) {     //  if image is large enough
								...
								$title = '';
								//  define title for this PDF image
								$name 	= $my_name;
								$title	= $my_name;
								...
								$id3_string = '';	//	as not available for images in PDFs
								...
								$sql_id3_string = '';	//	as not available for images in PDFs
								$sql_my_name    = $db_con->real_escape_string($my_name);
								...
								if (!$image_path) {
									...
									if ($width_p/$height_p > $ratio_orig) {
										$width = intval($height_p*$ratio_orig);
										$height	= $height_p;
									} else {
										$height = intval($width_p/$ratio_orig);
										$width	= $width_p;
									}
									...
									if($suffix == "jpeg" || $suffix == "jpg" || $suffix == "jif" || $suffix == "jpe"){
										...
										$im = @imagecreatefromgd($image_path);
									}
									else if($suffix == "gd2"){
										$im = @imagecreatefromgd2($image_path);
									}
									else if($suffix == "wbmp"){
										$im = @imagecreatefromwbmp($image_path);
									}

									// resample
									$image_p = imagecreatetruecolor($width, $height);
									imagecopyresampled($image_p, $im, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);
									$thumb_file = substr($image, 0, strrpos($image, '.'));
									...
									if (ImageTypes() & IMG_JPG) {           //  does gd-library support png ?
										$output = "$thumb_folder/".$dba_act."...".$mysql_table_prefix."...".$link_id."_-_".$media_id."-_-".$thumb_file.".jpg";  // create name for thumbnail
										imagejpeg($image_p,$output);         // make a .jpeg file
									} else {
									...
									if ($debug == '2') {
										if ($unreachable) {
											...
											if ($command_line ==1 || $cl == 1) { //  no thumbnail ouput for command line operation
												$all = "\r\n<br /> Title : $title_orig\r\n<br />Size : $size[0] x $size[1] pixel\r\n<br />Link : <a href='$link' target='rel' title='View image in new window'>".$link."</a>\r\n<br />No response, unreachable. $error\r\n<br />";
											} else {
												$all = "    <table width='100%'>
																<tr>
																	<td class='cntr x4'><a href='$link' target='rel' title='View image in new window'><img src=\"$output\" border=1 alt=\"Thumbnail\" /></a></td>
																	<td> Title :&nbsp;&nbsp;$title_orig<br /><br />Size :&nbsp;&nbsp;$size[0] x $size[1] pixel<br /><br />Link : <a href='$link' target='rel' title='View image in new window'>".$link1."</a><br /><br /><span class='warnadmin'>No response, unreachable. $error</span><br /></td>
																</tr>
															</table>
												";
											}
											...
											if ($command_line ==1 || $cl == 1) { //  no thumbnail ouput for command line operation
												$all = "Title : $title_orig\r\n<br />Size : $size[0] x $size[1] pixel<br />\r\nLink : <a href='$link' target='rel' title='View image in new window'>".$link1."</a>\r\n<br />";
											} else {
												$output = str_replace($inst_dir, $install_dir, $output);
												$all = "<table width='100%'>
	<tr>
		<td class='cntr x4'><a href='$link' target='rel' title='View image in new window'><img src=\"$output\" border=1 alt=\"Thumbnail\" /></a></td>
		<td> Title :&nbsp;&nbsp;$title_orig<br /><br />Size :&nbsp;&nbsp;$size[0] x $size[1] pixel<br /><br />Link : <a href='$link' target='rel' title='View image in new window'>".$link1."</a><br /></td>
	</tr>
</table>
";
											}
					...
					@unlink ($image_path);	//	no longer required
				}

			} else {
				//now all other images, not extracted from PDFs
...
//echo "\r\n\r\n<br /> im: '$im'<br />\r\n";
	...
	";
        ...
        sort($all_media);
        if ($clear == '1') {
            unset ($id3_string, $element, $thisimage, $title, $title_orig, $title_trans, $id3_rem, $name, $select, $all, $output);
        }
                    ...
                    if ($handle) {                      //  really existing video, or dead link only
                        ...
                        if ($result->num_rows && $new_md5) {   //      enter only, if PHP function md5_file()  is allowed for remote files
                            ...
                            if (!$result->num_rows) {                                       //  if this video is new
                                ...
                                //      output title, name and URL of actual video
                                $link1  = str_replace("%20", " ", $link);   //  make it readable
                                if ($debug == '2') {
                                    $link = str_replace($trash1, $replace1, $link);
...
//  ********** index Youtube videos **********
        if ($index_youtube == '1' && strstr($url, "youtube.com")) {

            if (preg_match("@watch\?v=@si", $url)) {  //  this looks like a Youtube video

                //get HTML body
                preg_match("@<body[^>]*>(.*?)<\/body\s*>@si",$raw_file, $regb);
                $raw_file       = $regb[1];
                $element        = 'video';
                $title          = '';
                $size           = '';
                ...
                //  get the detail row of this video
                preg_match("@<script>var ytplayer(.*?)<\/script\s*>@si",$raw_file, $regs);

                //  get title
                preg_match("@title\":\"(.*?)\",@si", $regs[1], $regt);
                $title  = $regt[1];

                //  get duration
                $regs_decoded = urldecode($regs[1]);
                preg_match("@dur=(.*?)(\.|\&)@si", $regs_decoded, $regdu);
                $duration   = $regdu[1];
                if (strpos($duration, ".")) {
                    $duration = substr($duration, 0, strpos($duration, "."));
                }
                if (is_int($duration) || ctype_digit($duration)) {
                    $id3_string     .= ">> duration ;; $duration seconds<br />";
                }

                $url1   = str_replace($trash1, $replace1, $url);
                $url2   = substr($url1, strrpos($url1, "/")+1);

                $sql_url        = $db_con->real_escape_string($url);
                $sql_title      = substr($db_con->real_escape_string($title), 0, 255);
                $sql_my_name    = substr($db_con->real_escape_string($title), 0, 1024);
                $sql_link       = $db_con->real_escape_string($url2);

                if ($index_id3 == 1) {
                    //  get size
                    preg_match("@size=(.*?)\\\u@si", $regs[1], $regr);
                    $size   = $regr[1];
                    if (strpos($size, ",")){
                        $size = substr($size, 0, strpos($size, ","));
                    }
                    $size_x = substr($size, 0, stripos($size, "x"));
                    $size_y = substr($size, stripos($size, "x")+1);

                    //  get fps
                    preg_match("@u0026fps=(.*?)\\\u@si",$raw_file, $regf);
                    if (is_int($regf[1]) || ctype_digit($regf[1])) {
                        $fps = $regf[1];
                    }

                    //  get video type
                    preg_match("@u0026type=(.*?)\\\u@si",$raw_file, $regty);
                    $mime_type  = substr(urldecode($regty[1]), 0, 35);
                    $suffix     = substr($mime_type, strpos($mime_type, "/")+1);
                    $suffix     = substr ($suffix, 0, strpos($suffix, ";"));
                    if (strlen($mime_type) < 10) {
                        $mime_type  = "video/mp4";  //  currently just a dummy output
                        $suffix     = "mp4";
                    }

                    //  get upload date
                    preg_match("@<strong class=\"watch-time-text\">(.*?)<\/strong>@si",$raw_file, $regsti);
                    $uploaded = substr($regsti[1], strrpos($regsti[1], " ")+1);

                    //  get description
                    preg_match("@<p id=\"eow-description\" >(.*?)<\/p>@si",$raw_file, $regsd);
                    $description = substr(str_replace("<br />", " ", $regsd[1]), 0, 2056);

                    //  now create ID3 string
                    $id3_string .= " >> url ;; $sql_url <br />";
                    $id3_string .= " >> suffix ;; $suffix <br />";
                    $id3_string .= " >> mime_type ;; $mime_type <br />";
                    $id3_string .= " >> fps ;; $fps <br />";
                    $id3_string .= " >> uploaded ;; $uploaded <br />";
                    $id3_string .= " >> size ;; $size pixel<br />";
                    $id3_string .= " >> size_x ;; $size_x pixel<br />";
                    $id3_string .= " >> size_y ;; $size_y pixel<br />";
                    $id3_string .= " >> full_name ;; $title <br />";
                    $id3_string .= " >> description ;; $description <br />";
                    //prepare all for database access
                    $sql_id3_string = $db_con->real_escape_string($id3_string);
                    $sql_suffix = $db_con->real_escape_string($suffix);
                }

                //      store all available information about actual Youtube video
                mysqltest();
                $sql_query = "INSERT into ".$mysql_table_prefix."media (link_id, link_addr, media_link, title, type, size_x, size_y, id3, md5sum, name, suffix) values ('$link_id', '$sql_url', '$sql_link', '$sql_title', '$element', '$size_x', '$size_y', '$sql_id3_string', '$new_md5', '$sql_my_name', '$sql_suffix')";
                $db_con->query ($sql_query);
                if ($debug && $db_con->errno) {
                    $file       = __FILE__ ;
                    $function   = __FUNCTION__ ;
                ...
                //  get thumbnail from this video
                preg_match("@<meta property=\"og\:image\" content=\"(.*?)\">@si", $regh[1], $regth);
                $thumb_url = $regth[1];

                mysqltest();
                $thumb_url = $db_con->real_escape_string($thumb_url);


                 if ($fp_remote = fopen($thumb_url, 'rb')) {
                    $localtempfile = tempnam('./tmp', 'img');        //  create name and path for temp image
                    if ($fp_local = fopen($localtempfile, 'wb')) {
                        while ($buffer = fread($fp_remote, 8192)) {    //  read remote image
                            //mysqltest();
                            if (fwrite($fp_local, $buffer) === FALSE) { //      write to local  temp image
                                $unreachable = '1';
                            }
                        }
                    }
                ...
                if (!$gd) {
                    ...
                    if (!$command) {    //  extract the suffix, but not for php command files
                        $exts = preg_split("{[/\\.]}", strtolower($thumb_url)) ;
                        $n = count($exts)-1;
                        $suffix = $exts[$n];
                        //$suffix = substr($thumb_url, strpos($thumb_url, "."));
                        if (strpos($suffix, "?")) {
                            $suffix = substr($suffix, 0, strpos($suffix, "?"));
                        }
                    }
                    ...
                    if (!$im) {
                        ...
                        $text_color = ImageColorAllocate ($im, 255, 0, 0);
                        ImageString ($im, 2, 8, 3, "No thumbnail available", $text_color);
                        ImageString ($im, 2, 14, 18, "for this Youtube,", $text_color);
                        ImageString ($im, 2, 16, 33, "video.", $text_color);
                        ImagePNG ($im,$output);
                        ...
                        $image_p = imagecreatetruecolor($width, $height);
                        imagecopyresampled($image_p, $im, 0, 0, 0, 0, $width, $height, $width_orig, $height_orig);
                        $name       = substr($thumb_url, 0, strrpos($thumb_url, "/"));
                        $thumb_file = substr($name, strrpos($name, "/")+1);
                        ...
                        // if no GD-library support for gif and png
                        if ($output =='') {
                ...
                //      output thumbnail, title, URL, etc. of actual Youtube video
                $link1  = str_replace("%20", " ", $sql_url);   //  make it readable
                $link  = str_replace("%25", "%", $link1);   //  make it readable

                if ($debug == '2') {
                    printStandardReport('newYoutubeVideo', $command_line, $no_log);
                    if ($unreachable) {
                        if ($unreachable == '1') $error = "Unable to write to temp-file.";
                        if ($unreachable == '2') $error = "Unable to open the remote thumbnail.";

                        if ($command_line ==1 || $cl == 1) { //  no thumbnail output for command line operation
                        ...
                        if ($command_line ==1 || $cl == 1) { //  no thumbnail output for command line operation
                            ...
                            $output = str_replace($inst_dir, $install_url, $output);
                            $all = "<table width='100%'>
<tr>
<td class='cntr x4'><a href='$thumb_url' target='rel' title='View video in new window'><img src=\"$output\" border=1 alt=\"Thumbnail\" /></a></td>
<td> Title :&nbsp;&nbsp;$title<br /><br />Size :&nbsp;&nbsp;$size[0] x $size[1] pixel<br /><br />Link : <a href='$url' target='rel' title='View link in new window'>".$url."</a><br /></td>
</tr>
</table>
";
                        }
        ...
        if ($clear == '1') {
            unset ($all_media, $id3_string, $element, $thisvideo, $thisaudio, $thisimage, $title, $title_orig, $title_trans, $id3_string, $id3_rem, $name, $select, $output);
            unset ($sql_title, $sql_description, $description, $sql_id3_string, $sql_suffix, $suffix, $mime_type, $uploaded, $size, $size_y, $size_x);
        }
        mysqltest();
    }

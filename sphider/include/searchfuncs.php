...
//      *******  finally 'STANDARD search'
					//$line = __LINE__;echo "\r\n<br>$line wordarray:<br><pre>";print_r($wordarray);echo "</pre>";
                    $words = 0;
                    while (($words < count($wordarray)) && $possible_to_find == 1) {
						...
						//$line = __LINE__;echo "\r\n<br>$line searchword: '$searchword'<br />\r\n";
                        $wordmd5 = substr(md5($searchword), 0, 1);

                        if ($sort_results == '7') {   //      get hit results
							$sql_query1 = "SELECT distinct * from ".$mysql_table_prefix."link_keyword$wordmd5, ".$mysql_table_prefix."keywords where ".$mysql_table_prefix."link_keyword$wordmd5.keyword_id= ".$mysql_table_prefix."keywords.keyword_id and keyword='$searchword' $domain_qry order by hits desc";
                        } else {                    // get weight results
							$sql_query1 = "SELECT distinct * from ".$mysql_table_prefix."link_keyword$wordmd5, ".$mysql_table_prefix."keywords where ".$mysql_table_prefix."link_keyword$wordmd5.keyword_id= ".$mysql_table_prefix."keywords.keyword_id and keyword='$searchword' $domain_qry order by weight desc";
                        }

						$result     = $db_con->query($sql_query1);
                        $num_rows   = $result->num_rows;
						//$line = __LINE__;echo "\r\n<br>$line num_rows: '$num_rows'<br />\r\n";
                        if ($type == "or" && $sort_results == '7') {
                            $indx = 0;
                        } else {
                            $indx = $words;
                        ...
                        if($num_rows != 0) {
                            while ($row = $result->fetch_array(MYSQLI_ASSOC)) {
                                ...
                                if ($sort_results == '7') {   //     7= sort results 'By hit counts in full text'  => ensure that result is also available in full_txt
                                    if ($type == 'phrase') {
                                        $searchword = $phrase_query;
                                    }
									//$linklist[$indx]['weight'][$row["link_id"]] = $row["hits"];      //  use hits
									$act =$row["link_id"];
									//$line = __LINE__;echo "\r\n<br>$line act link_id: '$act'<br />\r\n";
                                    $sql_query1 = "SELECT * FROM ".$mysql_table_prefix."links where link_id = '$act'";
                                    $reso = $db_con->query($sql_query1);
                                    $txt_res = $reso->fetch_array(MYSQLI_ASSOC);
                                    $full_txt = $txt_res["fulltxt"];          //       get fulltxt  of this link ID
									//$line = __LINE__;echo "\r\n<br>$line full_txt: '$full_txt'<br />\r\n";
                                    if ($case_sensitive == '0') {
                                        $full_txt = lower_case($full_txt);
                                    }
									...
									$searchword = implode(" ", $wordarray);		//	create string from all search words (like phrase search)
									//$line = __LINE__;echo "\r\n<br>$line searchword: '$searchword'<br />\r\n";
									//$line = __LINE__;echo "\r\n<br>$line full_txt: '$full_txt'<br />\r\n";
                                    if (substr_count($full_txt, $searchword)) {       //  ensure that query string was found in full_txt?
										$linklist_7[$indx]['weight'][$row["link_id"]] = $row["hits"];      //  use hits
                                        $linklist_7[$indx]['weight'][$row[0]] = substr_count($full_txt, $searchword);  //      number of hits found in this full_text
									}
                                }
								//$line = __LINE__;echo "\r\n<br>$line linklist_7:<br><pre>";print_r($linklist_7);echo "</pre>";
								if(count($linklist_7) >= 1){
									$linklist = $linklist_7;	// correct the linklist with results from hits in full_txt
								}
                            }
                        }
                        $words++;
                    }
					//$line = __LINE__;echo "\r\n<br>$line linklist Array 1:<br><pre>";print_r($linklist);echo "</pre>";
					...
					//$line = __LINE__;echo "\r\n<br>$line linklist Array 2:<br><pre>";print_r($linklist);echo "</pre>";
                    if ($translit_el) {
...
//  ***** end  different search modes
	   ...
	   $result_array_full = array();
        if ($words == 1 && $not_words == 0 && $category < 1 && $type !="phrase" || $sort_results == 7 ) { // for OR-Search and one word query, we already do have the result
		...
		//echo "\r\n\r\n<br>result_array_full 10:<br><pre>";print_r($result_array_full);echo "</pre>\r\n";
		...
		//echo "\r\n\r\n<br>result_array_full Array 20:<br><pre>";print_r($result_array_full);echo "</pre>\r\n";
       ...
       if ($result_array_full) {
		...
		//$line = __LINE__;echo "\r\n<br>$line result_array_temp :<br><pre>";print_r($result_array_temp);echo "</pre>";

		foreach ($result_array_temp as $key => $value){
            $result_array[$key] = $value;
            if (isset ($domains_to_show[$domains[$key]]) && $domains_to_show[$domains[$key]] != 1) {
				foreach ($domains_to_show[$domains[$key]] as $key => $value){
					$result_array[$key] = $value;
				}
            }
        }
		//$line = __LINE__;echo "\r\n<br>$line result Array 10:<br><pre>";print_r($result_array);echo "</pre>";
		...
		//echo "\r\n\r\n<br>result_array 1:<br><pre>";print_r($result_array);echo "</pre>\r\n";
        ...
        if (!is_array($in)) {
			...
			//$line = __LINE__;echo "\r\n<br>$line res Array:<br><pre>";print_r($res);echo "</pre>";
            return $res;
        }

		$inlist = implode(",", $in);
		$inlist = str_replace(",,", ",", $inlist);	// delete empty keys from $inlist array (shit happens)

		if (substr($inlist, -1) == ",") {
			$inlist = str_replace( ",", "", $inlist);	// delete final comma, if only single string in $inlist
		}
		//$line = __LINE__;echo "\r\n<br>$line inlist: '$inlist'<br />\r\n";
		...
		//$line = __LINE__;echo "\r\n<br>$line res array end of 'slave_search':<br><pre>";print_r($res);echo "</pre>";
        return $res;
    }
    ...
    function get_text_results($query, $start, $category, $searchtype, $results, $domain, $loop, $orig_query, $prefix) {
        ...
        global $db_con, $mysql_table_prefix, $desc_length, $phrase_desc, $case_sensitive, $debug, $debug_user, $charSet, $greek, $translit_el;
        global $use_cache, $textcache_dir, $tcache_size, $max_ctresults, $cn_seg, $dbu_act, $out, $xml_dir, $xml_name;
        global $most_pop, $pop_rows, $tag_cloud, $color_cloud, $catid, $db, $add_url, $shot_dir, $smt_button;
        ...
        global $cat_sel, $cat_sel0, $cat_sel0a, $cat_sel1, $cat_sel2, $cat_sel3, $cat_sel4, $cat_sel_all, $viking, $index_xml_pr, $pr_border;
...
//echo "\r\n\r\n<br>result Array:<br><pre>";print_r($result);echo "</pre>\r\n";
        ...
        if (isset($result['did_you_mean']) && $translit_el != '1') {
			...
			foreach ($result['did_you_mean'] as $key => $value){
                if ($key != $val) {
        ...
        if ($out == 'xml' && $smt_button != $sph_messages['m_search']) {    //  prepare the JSON and XML result files
...
//echo "\r\n\r\n<br /> thisword10: '$thisword'<br />\r\n";
					...
					foreach ($latin_ligatures as $key => $value){
                        $thisword1 = preg_replace("/".$key."/s", $value, $thisword); //  convert ligatures

                        if ($all_wild) {
                            $wild_liga  = preg_replace("/".$key."/s", $value, $all_wild); //  convert ligatures
                        }
					...
					foreach ($latin_ligatures as $key => $value){
                        $thisword1 = preg_replace("/".$value."/s", $key, $thisword); //  re-convert ligatures
                            if ($thisword1 != $thisword) {  //  break on first ligature
                                $thiswords['+'][] = html_entity_decode($thisword1, ENT_QUOTES, "UTF-8");
                            }
                        ...
                        if ($all_wild == $wild_orig) {
                            $wild_liga  = preg_replace("/".$value."/s", $key, $all_wild1); //  re-convert ligatures
...
//echo "\r\n\r\n<br /> shot: '$shot'<br />\r\n";
//echo "\r\n\r\n<br>words Array:<br><pre>";print_r($words);echo "</pre>\r\n";
//echo "\r\n\r\n<br>fulltxt Array:<br><pre>";print_r($fulltxt);echo "</pre>\r\n";
                ...
                if ($vowels == "1" && !preg_match("/[0-9]/",$words['hilight'][0])) {       //  remove accents
                    $tmp = remove_accents($fulltxt, '1');
                }
...
echo "\r\n\r\n<br /> tmp: '$tmp'<br />\r\n";
*/
				//	try to parse XML product feeds
				global $pr_feed;
				$pr_feed		= '';
				$all			= '';

				if ($index_xml_pr == 1 && strstr($url, ".xml") && strstr($fulltxt, "<br /><br /><br />")) {
					$pr_feed 		= 1;		// should be a result from an XML product feedd
					$desc_length 	= 9999999;	// increase the text length totally for XML product feeds

					// if selected in admin settings, extract only involved products in total XML product feed
					if ($pr_border == 1) {
						$found_in 	= '';
						$ip 		= '0';	//	counter for hits per product feed
						$extract  	= "<br /><br /><br />$fulltxt ";
						$extract1	= "<br /><br /><br />$tmp ";
//echo "\r\n\r\n<br /> extract aus fulltxt: '$extract'<br />\r\n";

//echo "\r\n\r\n<br /> extract aus fulltxt: '$extract'<br />\r\n";
//echo "\r\n\r\n<br /> extract1 aus tmp: '$extract1'<br />\r\n";

						$real_query	= str_replace("*", "", $words['+'][0]);
//echo "\r\n\r\n<br /> real_query: '$real_query'<br />\r\n";

/*
						//	bypass hits in links
						$found_test = mb_stripos($extract, $real_query);	// try to find first position of query

						while ($found_test != '' && $ip < $max_hits){
echo "\r\n\r\n<br /> found_test: '$found_test'<br />\r\n";
							$ip++;
							$before = mb_substr($extract, 0,  $found_test);
//echo "\r\n\r\n<br /> before: '$before'<br />\r\n";

							$link_b = strrpos($before, "<a href");	//	start position of last link before
							$link_e	= strrpos($before, "</a>");		//	end position of this link
//echo "\r\n\r\n<br /> link_b: '$link_b'<br />\r\n";
//echo "\r\n\r\n<br /> link_e: '$link_e'<br />\r\n";

							//	if $found_test was found inside of a link,
							//	create offset until the end of this link (ignore this hit)
							if ($link_b > 1  && $link_b < $found_test  && $link_e < $link_b){
								$end = mb_stripos($extract, "</a>", $found_test);	//	find the end of the involved link
								$offset = $end+4;
//echo "\r\n\r\n<br /> offset: '$offset'<br />\r\n";
/*
$sub = mb_substr($extract, 0, $offset);
echo "\r\n\r\n<br /> sub: '$sub'<br />\r\n";
*/
/*
							} else {
							...
							$found_test = mb_stripos($extract, $real_query, $offset);	// try to find next position of query
						}
*/
//echo "\r\n\r\n<br /> offset: '$offset'<br />\r\n";


						$found_in 	= mb_stripos($extract, $real_query, $offset);			//	find position of query string
//echo "\r\n\r\n<br /> found_in 0: '$found_in'<br />\r\n";
						$ip = 0;

						if ($found_in != '') {

							while ($found_in != ''&& $ip < $max_hits) {
								$ip++;
//echo "\r\n\r\n<br /> ip: '$ip'<br />\r\n";
//echo "\r\n\r\n<br /> extract4a: '$extract'<br />\r\n";
								$sub = mb_substr($extract, 0, $found_in);
//echo "\r\n\r\n<br /> sub: '$sub'<br />\r\n";
								$before	= mb_strrpos($sub, "<br /><br /><br />");			//	find attritutes beginning of this product
//echo "\r\n\r\n<br /> before: '$before'<br />\r\n";
								if ($before == '') {
									$before = "0";
								}
//echo "\r\n\r\n<br /> before1: '$before'<br />\r\n";
								$extract	= mb_substr($extract, $before);
								$extract 	= "   $extract";
//echo "\r\n\r\n<br /> extract2: '$extract'<br />\r\n";
								$end		= mb_stripos($extract, "<br /><br /><br />", 18);	//	find end of this product attributes
//echo "\r\n\r\n<br /> end: '$end'<br />\r\n";
								$extract	= mb_substr($extract, 0, $end);			//	total attributes of this product
//echo "\r\n\r\n<br /> extract3: '$extract'<br />\r\n";
								$att_length = strlen($extract);
								$all	.= "$extract <br /><br />";
//echo "\r\n\r\n<br /> all0: '$all'<br />\r\n";
								//$length = mb_strlen($all);
								//$endall = $length;						//	- 14 because '   <br /><br />' was added to $all
//echo "\r\n\r\n<br /> endall: '$endall'<br />\r\n";
								$extract_new = mb_substr($fulltxt,  mb_strpos($fulltxt, $extract)+$att_length-28);			//	in front of $fulltxt, $extract has been added by some characters (-4)
//echo "\r\n\r\n<br /> extract4: '$extract'<br />\r\n";
								$found_in = '';
								$found_in = mb_stripos($extract_new, $real_query);	//	find next position of query string
//echo "\r\n\r\n<br /> found_in 4: '$found_in'<br />\r\n";
								$all = str_replace("<br /> <br /><br /><br /><br />", "<br />", $all);	//	remove too many of them
								$extract = $extract_new;

//echo "\r\n\r\n<br /> all (Ende der while Schleife): '$all'<br />\r\n";
							}
						}

						$attr = $sph_messages['atts'];
						if ($ip == 1) {
							$attr = $sph_messages['att'];	// correct grammer
						}

						$fulltxt 	= $all;		//	for further processing (highlighting) we need it in $fulltxt
						$fulltxt 	= str_replace("<br /><br /><br />", "", $fulltxt);
						$fulltxt 	= str_replace("    <br />", " <br />", $fulltxt);
						$fulltxt 	= str_replace("    <br /><br />", " <br /><br />", $fulltxt);
						$fulltxt 	= str_replace("    ", " ", $fulltxt);
						$fulltxt	= "<strong>".$sph_messages['qfoundin']." $ip ".$attr."</strong><br /><br />$fulltxt";
						$tmp 		= str_replace("<br /><br /><br />", "<br /><br />", $tmp);	//correct the amount at the end of each product
						$tmp 		= str_replace("    ", " ", $tmp);
...
//echo "\r\n\r\n<br /> type: '$type'<br />\r\n";
				...
				if($type == 'phrase') {
					$words['+'] = $words['+s'];		//	replace single query terms with complete phrase
				}
...
//echo "\r\n\r\n<br>places Array1:<br><pre>";print_r($places);echo "</pre>\r\n";
					...
					if ($pr_feed != 1 && !strpos($url, ".xml")) {
						$this_text  = strip_tags($this_text);	// if not a result of an XML product feed
					}
                    ...
                    $word = $words['+'][0];
                    if ($phrase_desc == 1 || strpos($url, ".xml")) {
                        $begin      	= '0';
                        $dot        	= '';
                        $found_in   	= '';
						$phrase_desc	= '1';	//	redefine this variable for XML product feeds
                        if ($mb) {
                            $found_in   = mb_stripos($this_text, $word);    //  get position of keyword in current $this_text
                            $this_text1 = mb_substr($this_text, 0, $found_in);
                            $begin     = mb_strripos($this_text1, ". " );  //  find the last dot (and blank) in front of the keyword

                            if ($begin) {
                                if ($start >2) {
                                    $begin = $begin+2;  //  remove the leading dot and blank
                                } else {
                                    $begin = $begin+4;  //  whatever this may cause
                                }
                            } else {    //  if no dot and blank was found in front of the keyword

                                $begin = '0';
                                $this_text = "<strong>. . . </strong>$this_text";  //    no dot (and blank) found in front of keyword
                            }

                            $this_text = substr($this_text, $begin);
/*
echo "\r\n\r\n<br /> this_text2: '$this_text'<br />\r\n";
echo "\r\n\r\n<br /> word: '$word'<br />\r\n";
*/
                            //  recalculate position of first dot behind keyword
                            $found_in1  = mb_stripos($this_text, $word);
							if ($found_in1 == '') {
								$found_in1 = "15";		//	dummy offset if $thistext contains vovals, but $word not
							}

                            $dot        = mb_strpos($this_text, ". ", $found_in1); //  find next dot (and blank) behind keyword

                            if ($dot) {
                                $dot = $dot+1;      //  show the dot in text extract
                                $this_text = mb_substr($this_text, 0, $dot);     //  adjust text extract until dot
                            } else {
                                $this_text = mb_substr($this_text, 0, mb_strrpos($this_text, " "));
                                $this_text = "$this_text<strong> . . .</strong>";  //    no dot (and blank) found behind of keyword
                            }

                        } else {
                            $found_in   = stripos($this_text, $word);    //  get position of keyword in current $this_text
                            $this_text1 = substr($this_text, 0, $found_in);
                            $begin      = strripos($this_text1, ". " );  //  find the last dot (and blank) in front of the keyword
                            ...
                            //  recalculate position of first dot behind keyword
                            $found_in1  = stripos($this_text, $word);
                            $dot        = strpos($this_text, ". ", $found_in1); //  find next dot (and blank) behind keyword

                            if ($dot) {
                                $dot = $dot+1;      //  show the dot in text extract
                                $this_text = substr($this_text, 0, $dot);     //  adjust text extract until dot
                            } else {
...
//echo "\r\n\r\n<br /> text extract of first hit no. 0: '$this_text'<br />\r\n";
                        $actual_hit = "   <ul>
                                        <li>" . $this_text . "</li>
                        ";
                    ...
                    while ($place[$hit_id]) {
                        if ($hit_id <> $begin) {
                            ...
                            if($mb) {
                                $this_text = mb_substr($this_text, $begin_pos1, $desc_length);
                                $this_text = mb_substr($this_text, 0, mb_strrpos($this_text, " "));    //  find last blank

                                if ($phrase_desc) {
                                    $begin      = '0';
                                    $dot        = '';
                                    $found_in   = '';
                                    $found_in   = mb_stripos($this_text, $word);
                                    $this_text1 = mb_substr($this_text, 0, $found_in);

                                    $begin      = mb_strrpos($this_text1, ". ")+1;  //  find the last dot (and blank) in front of the keyword

                                    if ($begin == '1') {
                                        $this_text = " . . . $this_text";
                                    }
                                    $this_text = mb_substr($this_text, $begin);
                                    $found_in   = mb_stripos($this_text, $word);    //  get position of keyword in current $this_text

                                    $dot        = mb_strrpos($this_text, ". ", $found_in); //  find last dot (and blank) behind keyword

                                    if ($dot > 1) {
                                        $this_text  = mb_substr($this_text, 0, $dot);   //  limit the text extract at the last dot
                                    } else {
                                        $this_text = mb_substr($this_text, 0, mb_strrpos($this_text, " "));
                                        $this_text = "$this_text<strong> . . .</strong>";  //    no dot (and blank) found behind of keyword
                                    }
									...
									if ($phrase_desc == 1 || strpos($url, ".xml")) {
										$this_text .= "</a>";	// close eventually broken links in XML product feeds
									}
                                }
                                ...
                                if ($phrase_desc) {
                                    ...
                                    $dot        = '';
                                    $found_in   = '';
                                    $found_in   = stripos($this_text, $word);
                                    $this_text1 = substr($this_text, 0, $found_in);
                                    $dot        = strpos($this_text, ". ", $found_in)+1;   //  find next dot (and blank) behind keyword
                                    if ($dot) {
                                        $this_text  = substr($this_text, 0, $dot);      //  limit the text extract at the end
                                    } else {
                                        $this_text = mb_substr($this_text, 0, mb_strrpos($this_text, " "));
                                        $this_text = "$this_text<strong> . . .</strong>";  //    no dot (and blank) found behind of keyword
                                    }

                                    $begin = strripos($this_text, ". ")+2;      //  find the last dot (and blank) in front of the keyword
                    ...
                    //  remove invalid bold, because the leading dot before phrase around keyword was not found correctly
                    $actual_hit = preg_replace("@<li><strong>\.<\/li>@", "", $actual_hit);
...
//echo "\r\n\r\n<br /> fulltxt: '$fulltxt'<br />\r\n";
					...
					if ($pr_feed != 1 && !strpos($url, ".xml")) {
						$fulltxt  = strip_tags($fulltxt);	// if not a result of an XML product feed
					}
...
//echo "\r\n\r\n<br>places Array1:<br><pre>";print_r($places);echo "</pre>\r\n";
                    sort($places);
//echo "\r\n\r\n<br>places Array2:<br><pre>";print_r($places);echo "</pre>\r\n";
					if ($pr_feed == 1 ) {
						$places[0] = 1;
					}
//echo "\r\n\r\n<br>places Array3:<br><pre>";print_r($places);echo "</pre>\r\n";
				}
...
/*
echo "\r\n\r\n<br /> fulltxt100: '$fulltxt'<br />\r\n";
echo "\r\n\r\n<br>places Array final:<br><pre>";print_r($places);echo "</pre>\r\n";
echo "\r\n\r\n<br /> weight: '$weight'<br />\r\n";
*/
                    if (!$places[0] && $show_warning == '1' && $type !='tol' && !$only_links || ( $show_warning == '1' && $weight == '0')) {  // if  no keyword to be highlighted in fulltxt
                    ...
                    if($type == "phrase") {
                        ...
                        foreach($words['+'] as $change) {
							...
							if ($pr_feed == 1) {
								$fulltxt = str_replace (",", " ", $fulltxt);
								$fulltxt = preg_replace("/[\s]+/", " ", $fulltxt);     //  delete whitespaces
							}
                ...
                if ($out == 'xml' && $smt_button != $sph_messages['m_search']) {    //  prepare the XML result file
                    //  remove tags from title
                    $xml_title = preg_replace ("/<.*?>/", "", $title);
                    //$xml_title = str_replace ("</span>", "", $xml_title);
                    //$xml_title = str_replace ("<strong>", "", $xml_title);
                    //$xml_title = str_replace ("</strong>", "", $xml_title);
                    //  remove useless blanks
                    $xml_title  = preg_replace("/[  ]+/i", " ", $xml_title);

                    //  remove tags from fulltext
                    $xml_txt = preg_replace ("/<.*?>/", "", $fulltxt0);
                    //$xml_txt = str_replace ("</span>", "", $xml_txt);
                    //$xml_txt = str_replace ("<strong>", "", $xml_txt);
                    //$xml_txt = str_replace ("</strong>", "", $xml_txt);	//  remove trash
                    $xml_txt  = preg_replace("/[  ]+/i", " ", $xml_txt);	//  remove useless blanks
                    ...
                    $xml_result['text_results'][$i]['fulltxt']      =  preg_replace("@<.*?>@si", " ", $xml_txt);
...
//echo "<br>full_result Array:<br><pre>";print_r($full_result);echo "</pre>";
        ...
        if ($out == 'xml' && $smt_button != $sph_messages['m_search']) {    //  build the XML output file
...
//echo "\r\n\r\n<br>result Array:<br><pre>";print_r($result);echo "</pre>\r\n";
    ...
    function highlight($string, $change, $highlight) {
        global $case_sensitive, $mb, $vowels, $noacc_el, $translit_el, $home_charset, $strict_high, $pr_border, $pr_feed;

/*
echo "<br />----------------------------------------------<br />\r\n";
echo "\r\n\r\n<br /> string0: '$string'<br />\r\n";
echo "\r\n\r\n<br /> change: '$change'<br />\r\n";
echo "\r\n\r\n<br /> highlight: '$highlight'<br />\r\n";
echo "\r\n\r\n<br /> pr_feed: '$pr_feed'<br />\r\n";
echo "\r\n\r\n<br /> strict_high: '$strict_high'<br />\r\n";
*/
        if ($mb){
            @mb_internal_encoding("UTF-8");
			...
			if (strstr($string, "://")) {
				$strict_high = 1;									//	for links and XML product feeds highlight ony the search string (don't try to find the next 'blank')
			}

/*
			  else {
				$string = preg_replace("@<.*?>@si", " ",$string);   //remove all tags inside text extract
			}
*/
			...
			if ($case_sensitive == 1 ) {
				$string_ex = $string;	//	required for multiple results in one text string
                if ($vowels || $noacc_el || $translit_el) {

                    //  convert the text to be highlighted with respect to Admin settings
                    if($vowels) {
                        $string_ex  = remove_accents($string, '1');
                    } else {
                        $string_ex  = $string;
                    }
                    if($noacc_el) {
                    ...
                    $found1  = '';
                    $found2  = '';
                    $found1  = @mb_strpos($string_ex, $change);
//echo "\r\n\r\n<br /> found0: '$found1'<br />\r\n";
                    if ($translit_el) {
                        $found2  = @mb_strpos($string_ex, translit_el($change));
                        if (!$found1) $found1 = '999999999';
                        if (!$found2) $found2 = '999999999';
                        //  define  the minimum hit position
                        $found_in = min($found1, $found2);
                    ...
                    $found_in   = @mb_strpos($string, $change);
//echo "\r\n\r\n<br /> found0: '$found_in'<br />\r\n";
                    //  calculate length of string
                    if($strict_high) {
                        $length = (@mb_strlen($change)+ @mb_strlen($highlight))-1;    //  take into account the final blank
                    }else{
				...
				//while ($found_in !='' && $found_in < $txtlen) {         //  loop through full text
				while ($found_in !='') {         //  loop through full text
                    if ($change == "class") {
                        $string = str_replace("class=", "=ssalc", $string); //  replace 'class='
                    }

					$beginn_ex = @mb_substr($string_ex, 0, $found_in);		//  string until word to be highlighted
					$rest_ex   = @mb_substr($string_ex, $found_in);			//  rest of string incl. word to be highlighted

                    $beginn = @mb_substr($string, 0, $found_in);
                    $rest   = @mb_substr($string, $found_in);

					$string_ex	= "".$beginn_ex."<".$highlight.">".$rest_ex."";	//  include the highlight start-tag
					$string 	= "".$beginn."<".$highlight.">".$rest."";

					if ($strict_high) {
                        $end = $found_in+$length+3;		//   find end of word to be highlighted. +3 because < and > are added to $highlight + blank after $highlight
                    } else {
                        $end 	= @mb_strpos($string_ex, " ", $found_in+$length);  	//   find end of word (for combined words) to be highlighted.
                        $end1 	= @mb_strpos($string_ex, ".", $found_in+$length);	//   find end of word (for combined words like in URLs) to be highlighted.
                        $end2 	= @mb_strpos($string_ex, "<", $found_in+$length);
                        $end3 	= @mb_strpos($string_ex, "=", $found_in+$length);

						if ($end3 > 1 && $end3 < $end) {
                            $end = $end3;    //  dot was found before blank. Use dot position as end of highlighting
                        }

						if ($end2 > 1 && $end2 < $end) {
                            $end = $end2;    //  '<' was found before blank. Use '<' position as end of highlighting
                        }

						if ($end1 > 1 && $end1 < $end) {
                            $end = $end1;    //  '=' was found before blank. Use '.' position as end of highlighting
                        }
                    }

                    //  if no blank or dot was found
                    if ($end < 1 ) {
                        $ch_length  = @mb_strlen($change);
                        $length     = @mb_strlen($highlight)+2;      //  +2 because < and > was added to $highlight
                        $end        = $found_in+$length+$ch_length;
                    }

                    $rest_all_ex   	= @mb_substr($string_ex, $end);
                    $rest_all   	= @mb_substr($string, $end);

                    $string_ex	= "".@mb_substr($string_ex, 0, $end)."</span> ".$rest_all_ex."";    //  include highlight end-tag
                    $string     = "".@mb_substr($string, 0, $end)."</span> ".$rest_all."";    //  include highlight end-tag
					$offset     = $end+7 ;  //  +8 because </span> was added

					//  now try to find position of next hit with $offset as start position in $string
					$found_in   = '';
					$found1     = '';
					$found1_ex     = @mb_strpos($string_ex, $change, $offset);

					$pre = @mb_substr($string_ex, $found1_ex-1, 50);

					// use the new position for the 'while' loop
					$found_in	= $found1_ex;
//echo "\r\n\r\n<br /> found_in1: '$found_in'<br />\r\n";
                }
...
//echo "\r\n\r\n<br /> found0: '$found_in'<br />\r\n";
				...
				while ($found_in !='') {         //  loop through full text
					...
					if ($strict_high) {
                        ...
                        $end2 	= @mb_strpos($string_ex, "<", $found_in+$length);
                        $end3 	= @mb_strpos($string_ex, "=", $found_in+$length);
                        $end4 	= @mb_strpos($string_ex, ")", $found_in+$length);
/*
echo "\r\n\r\n<br /> end: '$end'<br />\r\n";
echo "\r\n\r\n<br /> end1: '$end1'<br />\r\n";
echo "\r\n\r\n<br /> end2: '$end2'<br />\r\n";
...
*/
						if ($end4 > 1 && $end4 < $end) {
                            $end = $end4;    //  dot was found before blank. Use dot position as end of highlighting
                        }
...
//echo "\r\n\r\n<br /> end all: '$end'<br />\r\n";
					...
					$found_in   = '';
					$found1     = '';
					$found1_ex     = @mb_stripos($string_ex, $change, $offset);
...
//echo "\r\n\r\n<br /> found_in1: '$found_in'<br />\r\n";
            ...
            $string = str_replace("<span</span>", "</span><span", $string); //  sometimes created by multiple hits per page
            ...
            return $string;		//	no highlighting without installed mb library
        }
    }

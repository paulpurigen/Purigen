<?php
    ...
    function image($query, $url, $media_type, $all, $urlx, $title1, $image_dir, $info, $info2, $thumb, $mode, $media_only, $type, $category, $catid, $mark, $db, $prefix, $domain) {
        ...
        if ($media_results) {
            ...
            $url = str_replace("https://www.youtube.com/", "", $url); //  remove this stupid Youtube preference
			...
			foreach($media_results as $value) {
                ...
                $link_crypt = str_replace("https://www.youtube.com", "", $link_crypt); //  remove this stupid Youtube preference

                $link_click = "$install_url/include/media_counter.php?url=$link_crypt&amp;query=$query&amp;db=$db&amp;prefix=$prefix&amp;client_ip=$client_ip";   //  redirect users click in order to update Most Media Popular Links
    ...
    function media($query, $url, $media_type, $all, $urlx, $title1, $image_dir, $info, $info2, $thumb, $mode, $media_only, $type, $category, $catid, $mark, $db, $prefix, $domain) {
        global $include_dir, $admin_dir, $sph_messages, $template, $template_dir, $index_id3, $limit_media, $delim, $install_url;
        ...
        if ($media_results) {
            ...
            //  as it is impossible to pass a youtube URL to another script,
            //  because the web browser takes over immediately
            $url1   = str_replace("https://www.youtube.com", "", $url);
            ...
            foreach($media_results as $value) {
                ...
                //      prepare current object-link for click counter
                if (strpos($value[2], "youtube")) {
                    $link_crypt  = str_replace("&", "-_-", $value[2]);
                } else {
                    $link_crypt = str_replace("&", "-_-", $value[3]);   //  crypt the & character
                }
                $link_crypt = str_replace("%25", "-__-", $link_crypt);  //  crypt the %25  because urldecode()  does not work
    ...
    function media_only($query, $start, $media_only, $type, $category, $catid, $mark, $db, $prefix, $domain) {
        global $debug, $debug_user, $admin_dir, $include_dir, $case_sensitive, $min_word_length;
        ...
        global $adv, $advanced_search, $show_media, $description, $embedded, $up_advanced, $thumb_dir;
        ...
        //error_reporting (E_ALL);      //  for debug only
         if (strlen($query) < $min_word_length){
            //   display header for media-only results and return without results
    ...
    ";
        ...
        //  if activated, prepare the JSON and XML result files
        if ($out == 'xml' && $smt_button == $sph_messages['m_search']) {
        ...
        if ($media_count > '0') {
           ...
           //      re-active default db for 'Search User'
            ...
            $row['category'] = '';
            if ($category != '-1') {
                $sql_query = "SELECT * from ".$mysql_table_prefix."categories                                               where category_id = '$catid'";
                ...
                if ($reso->num_rows) {
                    $row = $reso->fetch_array(MYSQLI_ASSOC);
                }
            }
            ...
            for ($i=$from; $i<$to; $i++) {
                ...
                //  build the thumbnail for Youtube videos
                if (strpos($this_media[2], "watch?v=")) {

                    $this_thumb = $this_media[4];  //  get thumb which will be shown by '120_media-only results.html'
                    $name       = substr($this_media[3], strpos($this_media[3], "v=")+2);   //  extract file name
                    $file       = utf8_decode("".$thumb_dir."/db".$db_slv."_".$mysql_table_prefix."_".$name.".gif");

                    //  work around, as urldecode() does not work here on some servers
                    ...
                    if (!fwrite($handle, $this_media[4])) {
                        if ($debug_user == '1') {
                            print "Unable to write the file $file. No thumbnails will be presented";
                        }
                    }
                    fclose($handle);
                    //  make the link to thumbnail HTTP compatible
                    $this_thumb = str_replace($inst_dir, $install_url, $file);
                }

                //      prepare current object-link for media counter
                if (strpos($this_media[2], "youtube" && $this_media[6] == "video")) {
                    $media_crypt  = str_replace("&", "-_-", $this_media[2]);
                } else {
                ...
                $media_click    = "$install_url/include/media_counter.php?url=$media_crypt&amp;query=$query&amp;db=$db&amp;prefix=$prefix&amp;client_ip=$client_ip";     //  redirect users click in order to update Most Popular Media

                //      prepare current page-link for media counter
                $link_crypt     = str_replace("&", "-_-", $this_media[2]);
                $link_click     = "$install_url/include/media_counter.php?url=$link_crypt&amp;query=$query&amp;db=$db&amp;prefix=$prefix&amp;client_ip=$client_ip";       //  redirect users click in order to update Most Popular Links
                $link_click = str_replace("https://www.youtube.com", "", $link_click); //  remove this stupid Youtube preference
    ...
    function thislink_media($query, $link, $media_type, $all, $domain, $db_slv, $mysql_table_prefix) {
        ...
        foreach ($media_results as $this_media[$i]) {
            ...
            if ($this_media[$i][6] == 'image') {
                ...
                if ($name == "default.jpg") {   //  might be multiple (Youtube), but must be individual for the thumbnail to be created
                    $name = rand().".jpg";
                }
            ...
            foreach ($all as $query) {      //  try to get results for any query word
                if ($search_id3 == '1') {    // search in name, title, EXIF and ID3 info
                    $sql_query ="SELECT * from ".$mysql_table_prefix."media
                                                    ...
                                                    OR id3 like LOWER           ('%".($db_con->real_escape_string($query))."%') $domain_qry
                                                    ORDER BY $sort ";
                    $result = $db_con->query($sql_query);
                    if ($debug && $db_con->errno) {
                        $file       = __FILE__ ;
        ...
        if ($cat_sel0 != $cat_sel_all  || $cat_sel1 != $cat_sel_all || $cat_sel2 != $cat_sel_all  || $cat_sel3 != $cat_sel_all  || $cat_sel4 != $cat_sel_all) {
			...
			foreach($all_media as $value) {
                ...
                if ($category_id) {
                    ...
                    //  if found in all active category selections, add the current link to the result array
                    if ($cat_to_find) {
                        $fresh_media[] = $value;
                    }
                }
...
//  special processing for images
        foreach ($fresh_media as $this_media[$i]) {
            if ($this_media[$i][6] == 'image') {        //  build thumbnail to be displayed in result listing
                ...
                if ($name == "default.jpg") {   //  might be multiple on Youtube, but must be individual for the thumbnail to be created
                    $name = rand().".jpg";
                }
                ...
                $fresh_media[$i][4] = $file;     //  replace content of thumbnail  with path to thumbnail
                $fresh_media[$i][5] = $title;    //  the title up to delimiter
                $i++;
            } else {
                $i++;
            }

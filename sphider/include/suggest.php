 ...
 ***********************************************************/
    ...
    $media_only = trim(substr(@$_GET['m_only'], 0, 1));
    $type       = trim(substr(@$_GET['type'], 0, 255));
...
$keyword = "option";
$media_only = "0";
$category = "-1";        //  -1      fetch without categories => present all suggestions
$catid = "2";
$type = "phrase";
$db = 0;
$prefix = 0;
...
*/
    ...
    //  save all input into tmp folder as 'suggest.txt'
    ...
    //      get active database for this application (defined by $dbu_act)
    //      here required to get the according variable $mysql_table_prefix (done by function db_connect(),
    //      and the according configuration file
    if ($dbu_act == '1') {
    ...
    //  get configuration file for this application defined by $dbu_act above
    $plus_nr = '';
    ...
    if ($quotes == '1') {
		...
		foreach ($all_quotes as $k => $v){
			$keyword = preg_replace("/".$all_quotes[$k]."/i", "$all_quotes[$v]", $keyword);
		}
    }
...
//  *****   enter here for media suggestions
        if ($media_only =='1') {
            ...
            //  get suggest results from all involved databases
            $media_results = array();
            if ($db1_slv == 1) {
                $db_con     = db_connect($mysql_host1, $mysql_user1, $mysql_password1, $database1);
                $mysql_table_prefix = $mysql_table_prefix1;
...
//   ********       enter here for text  suggestions
            ...
            if ($min_sug_chars != '0' && is_array($values)) {   // if we do have results, loop through them and add them to the output . Max. = $suggest_rows
				...
				//	remove special characters from suggestions
				$sug_array1 = array();
				foreach ($sug_array as $this_sug){
						$sug_array1[] = kill_secchars($this_sug);
					}
				$sug_array = array_unique($sug_array1);
                ...
                foreach ($sug_array as $this_sug){
                    ...
                    $sug_array = str_replace("\\", "", $sug_array);     // remove any backslash remained from MySQL real_escape_string
//echo "\r\n\r\n<br>sug Array:<br><pre>";print_r($sug_array);echo "</pre>\r\n";
                echo json_encode($sug_array);   //  let jQuery take over with these suggestions
...
*/
	...
	function kill_secchars($value){
            //    kill  special characters at the end of words
            $value = preg_replace('/—|\].|\%\?|\"\.|,|.\'|\.|\.\.|\.\.\.|!|\?|"|:|\)|\),|\)\.|】|）|？,|？|！|！|。,|。|„|“|”|”|”&nbsp;|»|\.»|;»|:»|,»|\.»|·»|«|«|»,|»\.|\.”|,”|;”|”\.|”,|‿|、|）|·|;|\]|\}|_,|_|”\)\.|.\">|\">|>|\)|&lt;|\%,|\%.|\%.\"|\%|\+\+|\+|\*|\#|\~|©|®|™/', "", $value);
            //    kill special characters in front of words
            $value = preg_replace('/—|\(\"|\(\$|\(\@|\@|\[|"|\(|„|„|“|（|«|【|‿|（|\(“|“|©|®|™|–|<|\/|\/|\\"|\.|\^|&gt;|\$|\£|\"\(|\+|\*|\.|\#|\%|\~|\{|»/', "", $value);
		return $value;
	}

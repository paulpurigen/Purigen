...
$dbhelper->dbOpen();
$sql = "select pkid, thumbnail1, thumbnail2, filename from events where pkid in( $pkid )";
...
if($List != null && count($List) > 0)
{
    ...
    {
        if($row["thumbnail1"] != "")
            $webhelper->DeleteFile("/events/", $row["thumbnail1"]);
        if($row["thumbnail2"] != "")
            $webhelper->DeleteFile("/events/", $row["thumbnail2"]);
        if($row["filename"] != "")
            $webhelper->DeleteFile("/events/", $row["filename"]);
        $sql = "delete from events where pkid = " . $row["pkid"];
        $dbhelper->RunSQL($sql);
    }
}
$dbhelper->dbClose();

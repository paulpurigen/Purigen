...
$dbhelper->dbOpen();
$sql = "select pkid, year, month, day, source, url, maintitle, subtitle, body, thumbnail, filename, orgfilename, status from press where pkid = $pkid ";
$ViewData = $dbhelper->RunSQLReturnOneRow($sql);
$dbhelper->dbClose();

if($webhelper->isNull($ViewData))
    $webhelper->AlertMessageAndGo("Error. Please try again.", "/admin/news/press/list.php" . $Parameter);
?>
<?php include_once($_SERVER['DOCUMENT_ROOT'] . '/admin/include/header.php')?>
    ...
    <script type="text/javascript">
        ...
        function del()
        {
            ...
            {
                f.action = "/admin/news/press/delete.php";
                f.submit();
            }
        }
    </script>
    ...
    <div id="container">
        ...
        <div id="contents">
            ...
            <form method="post" name="regFrm" action="/admin/news/press/modify.php" enctype="multipart/form-data">
                <input type="hidden" name="pkid"  value="<?=$ViewData["pkid"]?>"/>
                <input type="hidden" name="pkids[]"  value="<?=$ViewData["pkid"]?>"/>
                <input type="hidden" name="page"  value="<?=$page?>"/>
                <input type="hidden" name="searchKey"  value="<?=$searchKey?>"/>
                <input type="hidden" name="order"  value="<?=$order?>"/>
                <input type="hidden" name="oldthumbnail" value="<?=$ViewData["thumbnail"]?>" />
                ...
                <table class="board_write">
                    ...
                    <tr>
                        <th>source</th>
                        <td><input type="text" name="source" id="source" value="<?=$ViewData["source"]?>" maxlength="100" style="width:350px"></td>
                    </tr>
                    <tr>
                        <th>url</th>
                        <td><input type="text" name="url" id="url" value="<?=$ViewData["url"]?>" maxlength="200" style="width:350px"></td>
                    </tr>
                    <tr>
                        <th>main title</th>
                        <td><input type="text" name="maintitle" id="maintitle" value="<?=$ViewData["maintitle"]?>" maxlength="200" style="width:350px"></td>
                    </tr>
                    <tr>
                        <th>subtitle</th>
                    ...
                    <tr>
                        <th>body</th>
                        <td><textarea name="body" id="body" style="width:516px;height:280px"><?=$ViewData["body"]?></textarea></td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            ...
                            <?php
                                if( $ViewData["thumbnail"] != "" )
                                {
                                    ...
                                    echo "<img src=\"/files/press/" . $ViewData["thumbnail"] . "\">";
                                    echo "&nbsp;&nbsp;<input type=\"checkbox\" name=\"deleteThumbnail\" value=\"1\"/> Delete File\n";
                                    echo "</div>";
                                }
                            ?>
                        </td>

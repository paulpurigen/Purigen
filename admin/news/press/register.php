...
//file upload
$thumbnail = $webhelper->UploadFile("thumbnail", "/press/", true);
$file = $webhelper->UploadFile("up_file", "/press/");

$status = 0;

$dbhelper->dbOpen();
$sql = "insert into press(year, month, day, source, url, maintitle, subtitle, body, thumbnail, filename, orgfilename, status) values 
        ('$year', '$month', '$day', '$source', '$url', '$maintitle', '$subtitle', '$body', '" . $thumbnail["filename"] . "', '" . $file["filename"] . "', '" . $file["orgfilename"] . "', '$status' )";
$dbhelper->RunSQL($sql);
$dbhelper->dbClose();

$webhelper->AlertMessageAndGo("", "/admin/news/press/list.php");
?>
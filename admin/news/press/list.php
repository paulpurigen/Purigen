...
$dbhelper->dbOpen();
$sql = "select pkid, year, month, day, maintitle, source, status, thumbnail from press ";
if($searchKey != "")
{
...
<?php include_once($_SERVER['DOCUMENT_ROOT'] . '/admin/include/header.php'); ?>
    <script type="text/javascript">
        ...
        {
            location.href = "/admin/news/press/modifyFrm.php?pkid="+pkid+"&page=<?=$page.$Parameter?>";
        }
        ...
        function excute()
        {
            ...
            if(ischecked())
            {
                ...
                {
                    f.action = "/admin/news/press/publish.php";
                    f.submit();
                }else if($('#excute').val() == 3)
                {
                    f.action = "/admin/news/press/unpublish.php";
                    f.submit();
                }
            }else
            {
        ...
        function duplicate(pkid)
        {
            ...
            {
                location.href = "/admin/news/press/duplicate.php?pkid="+pkid+"&page=<?=$page.$Parameter?>";
            }
        }
    </script>
    ...
    <div id="container">
        ...
        <div id="contents">
            ...
            <form name="searchFrm" method="get" action="/admin/news/press/list.php">
                <input type="hidden" name="order" value="<?=$order?>"/>
                <div class="form_search">
                    <input type="text" name="searchKey" value="<?=$searchKey?>"> <a href="javascript:search();" class="bt bt-green">search</a>
                    <a href="/admin/news/press/registerFrm.php" class="bt bt-green add_new">ADD NEW</a>
                </div>
                <!-- //form_search -->
            </form>
            ...
            <form name="ListFrm" method="post">
                ...
                <table class="board_list">
                    ...
                    <tr>
                        ...
                        <th><a href="javascript:order(1);">date <?=$order == "1" ? "▲" : ($order == "2" ? "▼" : "")?></th>
                        <th><a href="javascript:order(3);">title <?=$order == "3" ? "▲" : ($order == "4" ? "▼" : "")?></a></th>
                        <th><a href="javascript:order(5);">source  <?=$order == "5" ? "▲" : ($order == "6" ? "▼" : "")?></a></th>
                    ...
                    <tr>
                        ...
                        <td><?=$row["day"]?> <?=$webhelper->GetMonthShortName($row["month"])?> <?=$row["year"]?></td>
                        <td><?=$row["maintitle"]?></td>
                        <td><?=$row["source"]?></td>

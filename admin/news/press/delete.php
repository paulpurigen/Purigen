...
$dbhelper->dbOpen();
$sql = "select pkid, thumbnail, filename from press where pkid in( $pkid )";
...
if($List != null && count($List) > 0)
{
    ...
    {
        if($row["thumbnail"] != "")
            $webhelper->DeleteFile("/press/", $row["thumbnail"]);
        if($row["filename"] != "")
            $webhelper->DeleteFile("/press/", $row["filename"]);
        
        $sql = "delete from press where pkid = " . $row["pkid"];
        $dbhelper->RunSQL($sql);
    }
}
$dbhelper->dbClose();

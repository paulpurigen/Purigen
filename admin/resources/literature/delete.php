...
$dbhelper->dbOpen();
$sql = "select pkid, thumbnail1, thumbnail2, filename from literature where pkid in( $pkid )";
...
if($List != null && count($List) > 0)
{
    ...
    {
        if($row["thumbnail1"] != "")
            $webhelper->DeleteFile("/literature/", $row["thumbnail1"]);
        if($row["thumbnail2"] != "")
            $webhelper->DeleteFile("/literature/", $row["thumbnail2"]);
        if($row["filename"] != "")
            $webhelper->DeleteFile("/literature/", $row["filename"]);
        
        $sql = "delete from literature where pkid = " . $row["pkid"];
        $dbhelper->RunSQL($sql);
    }
}
$dbhelper->dbClose();

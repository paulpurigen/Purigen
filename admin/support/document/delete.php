...
$dbhelper->dbOpen();
$sql = "select pkid, thumbnail1, thumbnail2, filename from document where pkid in( $pkid )";
...
if($List != null && count($List) > 0)
{
    ...
    {
        if($row["thumbnail1"] != "")
            $webhelper->DeleteFile("/document/", $row["thumbnail1"]);
        if($row["thumbnail2"] != "")
            $webhelper->DeleteFile("/document/", $row["thumbnail2"]);
        if($row["filename"] != "")
            $webhelper->DeleteFile("/document/", $row["filename"]);
        
        $sql = "delete from document where pkid = " . $row["pkid"];
        $dbhelper->RunSQL($sql);
    }
}
$dbhelper->dbClose();

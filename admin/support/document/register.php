...
$dbhelper->dbOpen();
$sql = "insert into document(type, year, month, day, title, body, thumbnail1, thumbnail2, filename, orgfilename, updatedate) values 
        ('$type', '$year', '$month', '$day', '$title', '$body', '" . $thumbnail1["filename"] . "','" . $thumbnail2["filename"] . "', '" . $file["filename"] . "','" . $file["orgfilename"] . "', now() )";
$dbhelper->RunSQL($sql);
$dbhelper->dbClose();

$webhelper->AlertMessageAndGo("", "/admin/support/document/list.php");
?>
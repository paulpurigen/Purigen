...
//file upload
$thumbnail1 = $webhelper->UploadFile("thumbnail1", "/document/", true);
if($thumbnail1 != null)
{ //upload new file so that delete old file;
    if($oldthumbnail1 != "")
        $webhelper->DeleteFile("/document/", $oldthumbnail1);
...
$thumbnail2 = $webhelper->UploadFile("thumbnail2", "/document/", true);
if($thumbnail2 != null)
{ //upload new file so that delete old file;
    if($oldthumbnail2 != "")
        $webhelper->DeleteFile("/document/", $oldthumbnail2);
...
$file = $webhelper->UploadFile("up_file", "/document/");
if($file != null)
{ //upload new file so that delete old file;
    if($oldfile != "")
        $webhelper->DeleteFile("/document/", $oldfile);
...
$webhelper->AlertMessageAndGo("", "/admin/support/document/list.php" . $Parameter);
?>
...
$dbhelper->dbOpen();
$sql = "select pkid, type, year, month, day, title, body, thumbnail1, thumbnail2, filename, orgfilename, updatedate from document where pkid = $pkid ";
$ViewData = $dbhelper->RunSQLReturnOneRow($sql);
$dbhelper->dbClose();

if($webhelper->isNull($ViewData))
    $webhelper->AlertMessageAndGo("Error. Please try again.", "/admin/support/document/list.php" . $Parameter);
?>
<?php include_once($_SERVER['DOCUMENT_ROOT'] . '/admin/include/header.php')?>
    ...
    <div id="container">
        ...
        <div id="contents">
            ...
            <form method="post" name="regFrm" action="/admin/support/document/modify.php" enctype="multipart/form-data">
                    ...
                    <tr>
                        ...
                        <td>
                            <select name="type" id="type">
                                <option value="0" <?=$webhelper->MakeSelectedValue("0", $ViewData["type"])?>>- select one -</option>
                                <option value="1" <?=$webhelper->MakeSelectedValue("1", $ViewData["type"])?>>SPECIFICATION SHEETS</option>
                                <option value="2" <?=$webhelper->MakeSelectedValue("2", $ViewData["type"])?>>USER GUIDES</option>
                                <option value="3" <?=$webhelper->MakeSelectedValue("3", $ViewData["type"])?>>SITE PREP GUIDES</option>
                                <option value="4" <?=$webhelper->MakeSelectedValue("4", $ViewData["type"])?>>PROTOCOLS</option>
                                <option value="5" <?=$webhelper->MakeSelectedValue("5", $ViewData["type"])?>>PRODUCT INSERTS</option>
                                <option value="6" <?=$webhelper->MakeSelectedValue("6", $ViewData["type"])?>>SAFETY DATA SHEETS</option>
                                <option value="7" <?=$webhelper->MakeSelectedValue("7", $ViewData["type"])?>>TERMS AND CONDITIONS</option>
                            </select>
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            ...
                            <?php
                                if( $ViewData["thumbnail1"] != "" )
                                {
                                    ...
                                    echo "<img src=\"/files/document/" . $ViewData["thumbnail1"] . "\">";
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php
                                ...
                                {
                                    echo "<div style=\"margin-top:10px\">";
                                    echo "<img src=\"/files/document/" . $ViewData["thumbnail2"] . "\">";

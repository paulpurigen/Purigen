...
$dbhelper->dbOpen();
$sql = "select pkid from email_list where pkid in( $pkid )";
...
if($List != null && count($List) > 0)
{
    foreach ($List as $row)
    {
        $sql = "delete from email_list where pkid = " . $row["pkid"];
        $dbhelper->RunSQL($sql);
    }
}
$dbhelper->dbClose();

$webhelper->AlertMessageAndGo("", "/admin/webforms/mailing/list.php" . $Parameter);
?>